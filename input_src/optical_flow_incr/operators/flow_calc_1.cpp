...
#pragma HLS interface axis register port=Output_2
  OUTER_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    ...
    OUTER_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      //out.val[0] = (x*x);
      //out.val[1] = (y*y);
      //out.val[2] = (x*y); // different from original
      //out.val[3] = (z*z); // different from original
      //out.val[4] = (x*z);
      //out.val[5] = (y*z);
...
static void tensor_weight_x_2(hls::stream< ap_uint<160> > &Input_1,
             hls::stream< ap_uint<160> > &Output_1)
{
// #pragma HLS interface axis register port=Input_1
// #pragma HLS interface axis register port=Output_1
...
static void tensor_weight_x_1(hls::stream< ap_uint<160> > &Input_1,
             hls::stream< ap_uint<160> > &Output_1)
{
// #pragma HLS interface axis register port=Input_1
// #pragma HLS interface axis register port=Output_1
  ...
  FLOW_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    FLOW_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      if(r>=2 && r<MAX_HEIGHT-2 && c>=2 && c<MAX_WIDTH-2)
      {
          ...
          //calc_pixel_t t4 = (calc_pixel_t) tmp_tensor.val[2]; // different from original
...
#pragma HLS dataflow
    ...
    tensor_weight_y_2(out_product2,t_w_y2_out);

    tensor_weight_x_1(t_w_y1_out,t_w_x1_out);
    tensor_weight_x_2(t_w_y2_out,t_w_x2_out);
