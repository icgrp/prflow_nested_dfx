...
#pragma HLS INTERFACE m_axi port=winner bundle=aximm3
	    ...
	    bit32 v3_buffer[64];   // Local memory to store vector3
		#pragma HLS STREAM variable=v3_buffer depth=64
...
#pragma HLS DATAFLOW
	    	...
	    	if(winner_size==0){
		    	...
		    	for(int i=0; i<winner_size; i++){ v3_buffer[i] = input3[i]; }
		    	for(int i=0; i<winner_size; i++){ winner[i] = v3_buffer[i]; }
	    	}
