...
#(parameter
    ...
    output wire [63:0]                   input3,
    output wire [63:0]                   winner,
    output wire [31:0]                   winner_size,
...
// 0x54 : reserved
// 0x58 : Data signal of input3
//        bit 31~0 - input3[31:0] (Read/Write)
// 0x5c : Data signal of input3
//        bit 31~0 - input3[63:32] (Read/Write)
// 0x60 : reserved
// 0x64 : Data signal of winner
//        bit 31~0 - winner[31:0] (Read/Write)
// 0x68 : Data signal of winner
//        bit 31~0 - winner[63:32] (Read/Write)
// 0x6c : reserved
// 0x70 : Data signal of winner_size
//        bit 31~0 - winner_size[31:0] (Read/Write)
// 0x74 : reserved
...
localparam
    ...
    ADDR_INPUT3_DATA_0      = 7'h58,
    ADDR_INPUT3_DATA_1      = 7'h5c,
    ADDR_INPUT3_CTRL        = 7'h60,
    ADDR_WINNER_DATA_0      = 7'h64,
    ADDR_WINNER_DATA_1      = 7'h68,
    ADDR_WINNER_CTRL        = 7'h6c,
    ADDR_WINNER_SIZE_DATA_0 = 7'h70,
    ADDR_WINNER_SIZE_CTRL   = 7'h74,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_input3 = 'b0;
    reg  [63:0]                   int_winner = 'b0;
    reg  [31:0]                   int_winner_size = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_INPUT3_DATA_1: begin
                    rdata <= int_input3[63:32];
                end
                ADDR_WINNER_DATA_0: begin
                    rdata <= int_winner[31:0];
                end
                ADDR_WINNER_DATA_1: begin
                    rdata <= int_winner[63:32];
                end
                ADDR_WINNER_SIZE_DATA_0: begin
                    rdata <= int_winner_size[31:0];
                end
            endcase
        end
    end
...
assign winner        = int_winner;
assign winner_size   = int_winner_size;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_INPUT3_DATA_1)
            int_input3[63:32] <= (WDATA[31:0] & wmask) | (int_input3[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WINNER_DATA_0)
            int_winner[31:0] <= (WDATA[31:0] & wmask) | (int_winner[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WINNER_DATA_1)
            int_winner[63:32] <= (WDATA[31:0] & wmask) | (int_winner[63:32] & ~wmask);
    end
end

// int_winner_size[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_winner_size[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WINNER_SIZE_DATA_0)
            int_winner_size[31:0] <= (WDATA[31:0] & wmask) | (int_winner_size[31:0] & ~wmask);
    end
end
